...
interface AsciiMoonCanvasProps {
  centerX?: number;
  centerY?: number;
  brightness?: number;
  className?: string;
...
/**
 * Campo ASCII fluido a pantalla completa con una luna 3D rotando.
 * Las corrientes horizontales reaccionan al mouse.
 */
export function AsciiMoonCanvas({
  centerX = 0.585,
  centerY = 0.34,
  brightness = 0.85,
  className = "absolute inset-0 h-full w-full",
}: AsciiMoonCanvasProps) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const moonX = width * centerX;
      const moonY = height * centerY;
      const moonRadius = Math.min(width, height) * 0.24;

      ctx.font = `${cellH * 0.84}px "Geist Mono", ui-monospace, monospace`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";

      // Dirección de luz de la luna
      let lx = 1.0;
      let ly = 0.15;
      let lz = -0.6;
      ...
      for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
          ...
          if (normMoon < 1.0) {
            ...
            // Rotación de la superficie lunar
            ...
            // Campo de corrientes horizontales
            const sampleX =
              c * 0.085 -
          ...
          ctx.fillStyle = `rgba(232, 230, 224, ${opacity * brightness})`;
